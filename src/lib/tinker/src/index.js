        ...
        fetchPromise
          .then(data => {
            ...
            if (data && data.status === 301) {
              ...
              let resultPromise = data.text();
              if (resultPromise) {
                resultPromise.then(data1 => {
                  const index = data1 && data1.indexOf('http');
                  result = data1 && data1.slice(index);
                  ...
                  if (isAndroid || isiOS) {
                    setupWebViewJavascriptBridge(function(bridge) {
                      bridge.callHandler("openurl", { url: result }, function(
                        response
                      ) {
                    ...
                    dispatch(push('/frame?src='+result));
                  }
                });
              }
              return { status: 0 };
