...
class LongHu extends React.PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      status: [],
    ...
    return (
      <div className={styles.longhuContent}>
        <div className={styles.longhuBody}>
          ...
          {this.state.status.indexOf(3) !== -1 ? 
            <div className={styles.onLonghuCard} onClick={() => {this.changeState(3)}}><img src={selectHe} className={styles.he} /></div> :
            <div className={styles.longhuCard} onClick={() => {this.changeState(3)}}><img src={he} className={styles.he} /></div>
          }
        </div>
      </div>
    );
